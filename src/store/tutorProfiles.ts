...
export const sSortOrder = signify<'asc' | 'desc'>('asc')
...
export const getAllTutorProfiles = async (params?: {
    ...
    try {
        ...
        const response = await tutorAPI.getTutorProfiles({
            page: params?.page || sPage.value,
            pageSize: params?.pageSize || sPageSize.value,
            sortOrder: params?.sortOrder || sSortOrder.value
        })

        const data = response.data as TutorProfile[]
        sTutorProfiles.set(data)
        sLoading.set(false)
